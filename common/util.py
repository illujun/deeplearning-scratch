...
def normalize(x):
    if x.ndim == 2:
        ...
        s = np.sqrt((x * x).sum())
        x /= s
    return x    # 1. 텍스트를 소문자로 변환하고 공백을 기준으로 단어 나누기
    text = text.lower()  # 모든 단어를 소문자로 변환
    words = text.split()  # 공백 기준으로 단어 나누기
    
    # 2. 단어에 고유한 ID 부여
    word_to_id = {word: i for i, word in enumerate(set(words))}  # 고유한 단어에 숫자 ID 부여
    id_to_word = {i: word for word, i in word_to_id.items()}  # ID에 해당하는 단어로 변환 딕셔너리
    
    # 3. corpus: 단어를 해당 ID로 변환한 리스트
    corpus = [word_to_id[word] for word in words]  # 각 단어를 ID로 변환
